class CreateAiAgentTables < ActiveRecord::Migration[7.0]
  ...
  def down
    drop_table :ai_agent_assistant_responses if table_exists?(:ai_agent_assistant_responses)
    drop_table :ai_agent_documents if table_exists?(:ai_agent_documents)
    drop_table :ai_agent_assistants if table_exists?(:ai_agent_assistants)
  ...
  def create_assistants
    create_table :ai_agent_assistants do |t|
    ...
    add_index :ai_agent_assistants, :account_id
    add_index :ai_agent_assistants, [:account_id, :name], unique: true
  end
  ...
  def create_documents
    ...
    add_index :ai_agent_documents, :assistant_id
    add_index :ai_agent_documents, [:assistant_id, :external_link], unique: true
  end

  def create_assistant_responses
    create_table :ai_agent_assistant_responses do |t|
    ...
    add_index :ai_agent_assistant_responses, :account_id
    add_index :ai_agent_assistant_responses, :assistant_id
    add_index :ai_agent_assistant_responses, :document_id
    add_index :ai_agent_assistant_responses, :embedding, using: :ivfflat, name: 'vector_idx_knowledge_entries_embedding', opclass: :vector_l2_ops
  end
