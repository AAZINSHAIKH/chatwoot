...
RSpec.describe AiAgent::InboxPendingConversationsResolutionJob, type: :job do
  ...
  let!(:ai_agent_assistant) { create(:ai_agent_assistant, account: inbox.account) }

  before do
    create(:ai_agent_inbox, inbox: inbox, ai_agent_assistant: ai_agent_assistant)
  ...
  it 'creates exactly one outgoing message with configured content' do
    ...
    ai_agent_assistant.update!(config: { 'resolution_message' => custom_message })
  ...
  it 'creates an outgoing message with default auto resolution message if not configured' do
    ai_agent_assistant.update!(config: {})
  ...
  it 'adds the correct activity message after resolution by AiAgent' do
    ...
    expect(activity_message.content).to eq(
      I18n.t('conversations.activity.ai_agent.resolved', user_name: ai_agent_assistant.name)
    )
  end
end
