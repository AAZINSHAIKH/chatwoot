...
RSpec.describe AiAgent::Documents::CrawlJob, type: :job do
  let(:document) { create(:ai_agent_document, external_link: 'https://example.com/page') }
  ...
  describe '#perform' do
    context 'when AI_AGENT_FIRECRAWL_API_KEY is configured' do
      ...
      context 'with account usage limits' do
        before do
          allow(account).to receive(:usage_limits).and_return({ ai_agent: { documents: { current_available: 20 } } })
        end
      ...
      context 'when crawl limit exceeds maximum' do
        before do
          allow(account).to receive(:usage_limits).and_return({ ai_agent: { documents: { current_available: 1000 } } })
        end
