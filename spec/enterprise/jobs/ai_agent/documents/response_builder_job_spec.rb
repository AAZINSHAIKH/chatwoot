...
RSpec.describe AiAgent::Documents::ResponseBuilderJob, type: :job do
  let(:assistant) { create(:ai_agent_assistant) }
  let(:document) { create(:ai_agent_document, assistant: assistant) }
  ...
  describe '#perform' do
    context 'when processing a document' do
      it 'deletes previous responses' do
        existing_response = create(:ai_agent_assistant_response, documentable: document)
