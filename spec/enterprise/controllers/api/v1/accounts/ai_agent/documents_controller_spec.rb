...
RSpec.describe 'Api::V1::Accounts::AiAgent::Documents', type: :request do
  ...
  let(:assistant2) { create(:ai_agent_assistant, account: account) }
  ...
  describe 'GET /api/v1/accounts/:account_id/ai_agent/documents' do
    ...
    context 'when it is an agent' do
      context 'when no filters are applied' do
        before do
          create_list(:ai_agent_document, 30, assistant: assistant, account: account)
        end
      ...
      context 'when filtering by assistant_id' do
        before do
          create_list(:ai_agent_document, 3, assistant: assistant, account: account)
          create_list(:ai_agent_document, 2, assistant: assistant2, account: account)
        end
        ...
        it 'returns empty array when assistant has no documents' do
          new_assistant = create(:ai_agent_assistant, account: account)
          get "/api/v1/accounts/#{account.id}/ai_agent/documents",
      ...
      context 'with pagination and assistant filter combined' do
        before do
          create_list(:ai_agent_document, 30, assistant: assistant, account: account)
          create_list(:ai_agent_document, 10, assistant: assistant2, account: account)
        end
  ...
  describe 'POST /api/v1/accounts/:account_id/ai_agent/documents' do
    ...
    context 'when it is an admin' do
      ...
      context 'with limits exceeded' do
        before do
          create_list(:ai_agent_document, 5, assistant: assistant, account: account)
  ...
  describe 'DELETE /api/v1/accounts/:account_id/ai_agent/documents/:id' do
    ...
    context 'when it is an agent' do
      let!(:document_to_delete) { create(:ai_agent_document, assistant: assistant) }
