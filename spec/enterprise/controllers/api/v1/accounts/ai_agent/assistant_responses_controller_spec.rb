...
RSpec.describe 'Api::V1::Accounts::AiAgent::AssistantResponses', type: :request do
  ...
  let(:document) { create(:ai_agent_document, assistant: assistant, account: account) }
  let(:admin) { create(:user, account: account, role: :administrator) }
  let(:agent) { create(:user, account: account, role: :agent) }
  let(:another_assistant) { create(:ai_agent_assistant, account: account) }
  let(:another_document) { create(:ai_agent_document, account: account, assistant: assistant) }

  def json_response
    JSON.parse(response.body, symbolize_names: true)
  end

  describe 'GET /api/v1/accounts/:account_id/ai_agent/assistant_responses' do
    context 'when no filters are applied' do
      before do
        create_list(:ai_agent_assistant_response, 30,
                    account: account,
                    assistant: assistant,
      ...
      it 'returns first page of responses with default pagination' do
        get "/api/v1/accounts/#{account.id}/ai_agent/assistant_responses",
    ...
    context 'when filtering by assistant_id' do
      before do
        create_list(:ai_agent_assistant_response, 3,
                    account: account,
                    assistant: assistant,
                    documentable: document)
        create_list(:ai_agent_assistant_response, 2,
                    account: account,
  ...
  describe 'GET /api/v1/accounts/:account_id/ai_agent/assistant_responses/:id' do
    let!(:response_record) { create(:ai_agent_assistant_response, assistant: assistant, account: account) }

    it 'returns the requested response if the user is agent or admin' do
      get "/api/v1/accounts/#{account.id}/ai_agent/assistant_responses/#{response_record.id}",
          headers: agent.create_new_auth_token,
          as: :json
  ...
  describe 'POST /api/v1/accounts/:account_id/ai_agent/assistant_responses' do
    let(:valid_params) do
      {
        assistant_response: {
    ...
    it 'creates a new response if the user is an admin' do
      expect do
        post "/api/v1/accounts/#{account.id}/ai_agent/assistant_responses",
             params: valid_params,
  ...
  describe 'PATCH /api/v1/accounts/:account_id/ai_agent/assistant_responses/:id' do
    let!(:response_record) { create(:ai_agent_assistant_response, assistant: assistant) }
    ...
    it 'updates the response if the user is an admin' do
      patch "/api/v1/accounts/#{account.id}/ai_agent/assistant_responses/#{response_record.id}",
  ...
  describe 'DELETE /api/v1/accounts/:account_id/ai_agent/assistant_responses/:id' do
    let!(:response_record) { create(:ai_agent_assistant_response, assistant: assistant) }

    it 'deletes the response' do
      expect do
        delete "/api/v1/accounts/#{account.id}/ai_agent/assistant_responses/#{response_record.id}",
               headers: admin.create_new_auth_token,
               as: :json
      end.to change(AiAgent::AssistantResponse, :count).by(-1)
    ...
    context 'with invalid id' do
      it 'returns not found' do
        delete "/api/v1/accounts/#{account.id}/ai_agent/assistant_responses/0",
               headers: admin.create_new_auth_token,
               as: :json
