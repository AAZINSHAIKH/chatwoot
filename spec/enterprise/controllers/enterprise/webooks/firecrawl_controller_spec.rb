  ...
  describe 'POST /enterprise/webhooks/firecrawl?topic_id=:topic_id&token=:token' do
    ...
    context 'with valid token' do
      context 'with crawl.page event type' do
        ...
        it 'processes the webhook and returns success' do
          ...
          post(
            "/enterprise/webhooks/firecrawl?topic_id=#{assistant.id}&token=#{valid_token}",
            params: valid_params,
            as: :json
          )
      ...
      context 'with crawl.completed event type' do
        ...
        it 'returns success without enqueuing job' do
          ...
          post("/enterprise/webhooks/firecrawl?topic_id=#{assistant.id}&token=#{valid_token}",
               params: valid_params,
               as: :json)
    ...
    context 'with invalid token' do
      ...
      it 'returns unauthorized status' do
        post("/enterprise/webhooks/firecrawl?topic_id=#{assistant.id}&token=invalid_token",
