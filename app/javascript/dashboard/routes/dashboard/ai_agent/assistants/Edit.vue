...
import EditAssistantForm from '../../../../components-next/ai_agent/pageComponents/assistant/EditAssistantForm.vue';
import AssistantPlayground from 'dashboard/components-next/ai_agent/assistant/AssistantPlayground.vue';
...
const assistant = computed(() =>
  store.getters['ai_agentAssistants/getRecord'](Number(assistantId))
);
...
const handleSubmit = async updatedAssistant => {
  try {
    await store.dispatch('ai_agentAssistants/update', {
      id: assistantId,
      ...updatedAssistant,
    });
...
onMounted(() => {
  if (!isAssistantAvailable.value) {
    store.dispatch('ai_agentAssistants/show', assistantId);
  }
});
</script>
...
<template>
  <PageLayout
    ...
    :back-url="{ name: 'ai_agent_assistants_index' }"
  >
    <template #body>
