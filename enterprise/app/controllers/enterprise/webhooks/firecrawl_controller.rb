class Enterprise::Webhooks::FirecrawlController < ActionController::API
  ...
  def process_payload
    Captain::Tools::FirecrawlParserJob.perform_later(topic_id: assistant.id, payload: payload) if crawl_page_event?
  ...
  def assistant
    @assistant ||= Captain::Topic.find(permitted_params[:topic_id])
  end
