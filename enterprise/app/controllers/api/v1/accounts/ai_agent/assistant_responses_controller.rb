class Api::V1::Accounts::AiAgent::AssistantResponsesController < Api::V1::Accounts::BaseController
  ...
  def create
    @response = Current.account.ai_agent_assistant_responses.new(response_params)
  ...
  def set_assistant
    @assistant = Current.account.ai_agent_assistants.find_by(id: params[:assistant_id])
  end

  def set_responses
    @responses = Current.account.ai_agent_assistant_responses.includes(:assistant, :documentable).ordered
  end
